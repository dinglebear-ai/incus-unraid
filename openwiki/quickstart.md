...
description: Entry-point guide for the incus-unraid repository, summarizing runtime architecture, operations, and where source files and workflows live.
tags: [incus, unraid, plugin, openwiki]
---
...
- `/CLAUDE.md` — Agent operating notes, including OpenWiki update policy and API-plugin architecture reminders

## OpenWiki maintenance

- Scheduled OpenWiki updates are driven by `/.github/workflows/openwiki-update.yml`, which now runs `openwiki code --update --print` directly with OpenRouter (`OPENWIKI_PROVIDER=openrouter`) instead of the previous Tailscale-backed OpenAI-compatible endpoint.
- The workflow passes `OPENWIKI_MODEL_ID=z-ai/glm-5.2` and tracing metadata via `LANGSMITH_API_KEY`/`LANGCHAIN_PROJECT`/`LANGCHAIN_TRACING_V2`.
- PRs generated by the workflow include changes to `openwiki/`, `AGENTS.md`, `CLAUDE.md`, and `/.github/workflows/openwiki-update.yml`.
- `CLAUDE.md` includes the repository policy that generated OpenWiki pages are normally refreshed via this workflow rather than manually edited.
